...
Current projection after read-only F01/F02 evidence acquisition. GraphHopper functional PASS (boot persistence pending). nginx/ORS/GIS/Nav/D-Flight remain inactive. Two configuration gaps are now proven: nginx GOI vhost is staged but not included; served GIS HTML still targets OLD GOI endpoints.
...
| nginx GOI vhost | OLD live TS `:443` + public default `:80` | intended NEW vhost exists only in `/etc/nginx/sites-available/goi-ors-gateway`; `sites-enabled` and `conf.d` empty; current `nginx -t` does **not** test the staged GOI vhost | Tailscale/TLS/ORS | PRESENT_NOT_VALIDATED | **F01 include remediation required before activation** |
