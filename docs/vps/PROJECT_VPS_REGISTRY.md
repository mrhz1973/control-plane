...
| Automazione Posta-Documenti-Gdrive | none | R002 bootstrap APPLIED live 2026-09-19: dedicated DB `maildocs`, role `maildocs_app`, schema `maildocs` on `root-postgres-1`; migration `001_core` applied with idempotent double-run PASS; runtime root `/root/local-files/handoff-runtime/automazione-posta-documenti-gdrive` (repo checkout `007b895`); no listener/public port | PostgreSQL, n8n, `/root/local-files` | DEPLOYED_VALIDATED | #107; app repo issue #2; `reports/architecture/r002_maildocs_vps_db_bootstrap_evidence_v1.md` |
