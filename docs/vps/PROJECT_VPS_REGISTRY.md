...
| Automazione Posta-Documenti-Gdrive | none | R002 bootstrap APPLIED live 2026-09-19: dedicated DB `maildocs`, role `maildocs_app`, schema `maildocs` on `root-postgres-1`; migration `001_core` applied with idempotent double-run PASS; runtime root `/root/local-files/handoff-runtime/automazione-posta-documenti-gdrive` (repo checkout `007b895`); R002A 2026-09-19: n8n credential `Maildocs PostgreSQL — maildocs_app` (type `postgres`, id `aw36feohto4X9BPz`) created+owned by personal project, connectivity smoke PASS as `maildocs_app`, DELETE/TRUNCATE denial re-proven, active workflows unchanged 4/4; temp smoke workflow left INACTIVE (CLI delete unavailable in n8n 2.33.3) | PostgreSQL, n8n, `/root/local-files` | DEPLOYED_VALIDATED | #107; #111; app repo issues #2/#3; `reports/architecture/r002_maildocs_vps_db_bootstrap_evidence_v1.md`; `reports/architecture/r002a_maildocs_n8n_postgres_credential_smoke_evidence_v1.md` |
