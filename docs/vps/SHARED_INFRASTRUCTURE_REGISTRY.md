...
| nginx global service | Control Plane | active/enabled OLD | installed, active not enabled NEW; bind Tailscale `:443` only | no enable/public cutover |
...
| GOI D-Flight | GOI / Control Plane activation gate | OLD TS `:8010` + persistent state | config DAC remediations PASS (`root:goi-dflight`); unit still inactive; GIS client NEW D-Flight destination | retry functional qualification; no start in permission pass |
...
| GOI TS-bound ports | Control Plane allocates; GOI validates | OLD `443,5000,8000,8010,8989`; loopback `8020,8990` | NEW GraphHopper `8989` + admin `8990`; ORS loopback `8020`; nginx TS `443`; GIS/Nav/D-Flight still closed | never expose `8020/8990` beyond loopback |
