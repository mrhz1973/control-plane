...
| n8n loopback `5678` | Control Plane | OLD writer stopped/frozen; DB/publication retained | NEW live, loopback only; active map = **4 Control Plane unchanged + 1 Maildocs WF01** (total 5) after R003D PASS | rollback retention; no dual writer; Maildocs deactivate = unpublish WF01 only |
...
| Maildocs Gmail n8n credential + shadow WF01 | Automazione-Posta-Documenti-Gdrive / Control Plane gate | n/a | DEPLOYED_VALIDATED 2026-09-20: credential `gmailOAuth2` id `xz6lufyo9JjjBG6i` name `Maildocs Gmail — READ ONLY` scope `gmail.readonly`; workflow `WF01GmailShadowR003B` **ACTIVE** ScheduleTrigger every 15m + terminal-skip `CLASSIFIED_SHADOW` (R003D / #122); durable shadow → `maildocs`; natural tick `357590` PASS; no Gmail mutation; no public bind; CP 4 active workflows unchanged | #117/#119/#122 + app #8/#9/#10; evidence `r003b_…` + `r003c_…` + `r003d_maildocs_gmail_readonly_scheduled_polling_activation_evidence_v1.md` |
