...
## Authorized decommission execution — 2026-09-14 (`V4_VPS_68_AUTHORIZED_OLD_DECOMMISSION_EXECUTION_V1`)

Operator authorization recorded on #68 (issuecomment-5665517718):
`AUTORIZZO LA DISMISSIONE DEL VECCHIO VPS`.

```text
HUMAN_OLD_DECOMMISSION_AUTHORIZATION=YES
OLD_IDENTITY_FENCE=PASS (alias→217.160.71.145; hostname=ubuntu; egress=217.160.71.145; Tailscale=100.114.7.53; n8n inactive — frozen standby; TARGET_IS_NEW=NO)
IDENTITY_RECHECK_BEFORE_DESTRUCTIVE=PASS
FINAL_BACKUP_EVIDENCE_REQUIREMENT=SATISFIED_BY_EXISTING_CANONICAL_EVIDENCE (write-freeze dump + SHA256 + NEW pre-restore backup + final sync + publication map + sequence audit + soak; no duplicate dump made)
OLD_OS_SHUTDOWN=PASS (systemctl poweroff; SSH + ports 22/443 externally unreachable)
OLD_ROLE=DECOMMISSIONED_OS_SHUTDOWN_COMPLETE_PROVIDER_TERMINATION_PENDING
OLD_PROVIDER_TERMINATION=MANUAL_OPERATOR_ACTION_REQUIRED (no already-authenticated deterministic IONOS control surface exists: no ionosctl, no API credentials, no browser scraping by law)
OLD_DECOMMISSION_AUTHORIZED=YES
OLD_DECOMMISSION_EXECUTED=PARTIAL_OS_SHUTDOWN_COMPLETE
ROLLBACK_RETENTION=CLOSED_BY_OPERATOR_AUTHORIZATION
NEW_POST_OLD_SHUTDOWN_SANITY=PASS (egress 31.70.139.73; Tailscale 100.99.54.93; docker root-n8n-1 Up 6 days; root-postgres-1 Up 7 days healthy; n8n /healthz {"status":"ok"})
NEW_ROLE=LIVE (unchanged; no NEW mutation)
TAILSCALE_OLD_NODE_CLEANUP=NONBLOCKING_MANUAL_HYGIENE (node ubuntu/100.114.7.53 now offline; remove via Tailscale admin when convenient; never touch NEW identity)
