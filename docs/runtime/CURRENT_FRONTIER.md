...
| **QUOTA_AWARE_RUNTIME** | **RUNTIME_WIRED_BEHIND_CLOSED_GATE (canonical entrypoints + CLI wired — CLI/mixed-route fix PASS)** · RT25 campaign 25/25 (historical, `reports/architecture/v4_rt25_quota_aware_runtime_wiring_closure_v1.md`) **+** correction `V4_RT25_CANONICAL_ENTRYPOINT_INTEGRATION_CORRECTION_V1` (base `0ec7826`): canonical producer `tools/rt25-canonical-quota-state-v1.mjs` (real registry-v2 + real fail-closed baseline + real ingest-lane contributions → real composer → real join) consumed INSIDE `tools/evaluate-planner-selection.mjs` (via `prepareCycle(quotaStateOptions)` canonical upstream, fail-closed `QUOTA_STATE_COMPOSITION_FAILED`) and INSIDE `tools/evaluate-execution-route.mjs` (stage-5.5 pool admission; router EMITS the RT25 envelope mirroring its final route) → n8n bridge consumes the ROUTER-PRODUCED envelope automatically (`options.quotaStateOptions`, `QUOTA_DECISION_PRODUCED_BY_CANONICAL_ROUTER`) → Windows endpoint validates provenance **+** fix `V4_RT25_CANONICAL_CLI_AND_MIXED_ROUTE_FIX_V1` (base `5322b31`): EXISTING CLI shapes are quota-aware with no new flag — WF61 `prepare` (no quota flag) and WF40 bridge (cycle/route/status only) auto-compose the canonical quota state from the standard ingest lane; explicit invalid `quotaStateOptions` (null/false/string/array) fail closed `QUOTA_STATE_OPTIONS_INVALID` (no legacy bypass); planner denial law hardened (ANY pool-admission denial → `UNAVAILABLE`, reason kept in `quota_pool_refinements`; legacy CONSERVE can no longer PROCEED a denied commercial pool); mixed-route temporal-dead-zone fixed (`reasonAccum` hoisted above stage 5.5 → `QUOTA_POOL_NARROWED` recorded, commercial rejected + valid Qwen local survives, no exception); selector `pool_evaluations` carries denied pools' fail-closed evaluation (all-rejected no longer empty); bridge keeps router quota provenance in `ADAPTER_REGISTRY_INVALID`/`ADAPTER_NOT_REGISTERED` outcomes · proofs: `tests/rt25-canonical-cli-wiring` **31/31** (exact WF61/WF40 CLI, absent/empty/stale/fresh lane, narrowing, endpoint validator, bypass prevention, legacy imports) + `tests/rt25-canonical-entrypoint-wiring` **104/104** (laws A..N incl. M mixed-route narrowing + N no-legacy-fallback) + `tests/rt25-canonical-closed-gate-e2e` **12/12** + full regression 38 suites green · BUGBOT_REVIEW=FINDINGS_FIXED (2 findings on PRE-EXISTING untracked files — excluded from commit, never staged/run) · shared pools: `chatgpt_codex_subscription` (subscription-only), `glm_coding_plan` (5.3+Flash one pool) · reviewer/retry canonical boundary MISSING — #43 micro-task re-verified absence at `8789b23` (STOP persisted: 0 runtime callers of the T18 selector / producer binding, `packet.review` never read behaviorally by the policy gate, no review stage in WF40/WF61/local-dev; wiring requires a governed review-stage architecture (a)/(b)/(c), not invention) · GLM live collector **BLOCKED_EVIDENCE** (credential absent → UNKNOWN fail-closed) · no production route activation; bridge `dispatch_prepared=false` invariant intact · correction report `reports/architecture/v4_rt25_canonical_entrypoint_integration_correction_v1.md` |
