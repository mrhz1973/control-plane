...
| **BLOCCO ATTIVO** | `V4_LOCAL_DEV_EXECUTOR_QUEUE_CLAIM_SELECTION_V1` — **PASS** · selector deterministico `tools/select-local-dev-queue-item-v1.mjs` (stessi gate del bridge, receipts single-writer, ordering risk→FIFO) · suite: 10/10 legge selezione + 6/6 CLI · **FIRST LIVE SELECTED-ITEM DISPATCH = PASS** (executor `9eccd65`): selector→claim (receipts.json)→executor→OpenCode→Qwen Cline24K→MODIFY tracked in-scope (`docs/runtime/CAMPAIGN_NOTES.md`)→push→remote-verify · 4 turni / 171s · preexisting_untracked_protected=38 · entrambi i path CREATE (checkpoint 7) e MODIFY (checkpoint 8) ora provati live end-to-end |
...
| **NEXT** | derivato AUTO-VIA (segment 3, dopo checkpoint 8): `V4_LOCAL_DEV_EXECUTOR_DISPATCHER_LOOP_V1` (proposto) — primitiva dispatcher non presidiata: ciclo `select → bridge-claim → dispatch → persist receipts` come unico comando ripetibile con limiti espliciti (max N task, stop su primo STOP sostanziale), test offline + un bounded live proof; resta dentro LOCAL_DEV, nessuna produzione/n8n |
