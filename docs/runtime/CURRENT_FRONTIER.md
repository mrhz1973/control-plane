...
| **STATO BLOCCO** | `D0016_W_PHASE_A_READONLY_DISCOVERY_READY / WINDOWS_FALLBACK_READY / ZAI_SUPPORT_WAIT_PARALLEL` |
| **GATE CORRENTE** | `D0016_W_PHASE_A_READONLY_DISCOVERY` — inspect the existing home Windows OpenClaw host only; no provider inference and no config mutation |
| **NEXT** | Cursor on the home Windows OpenClaw host syncs repo safely, inspects sanitized Gateway HTTP endpoint metadata, verifies loopback/Tailscale/auth-mode invariants, and if an OpenAI-compatible surface is already enabled may perform metadata-only authenticated `GET /v1/models`; provider/model inference request count must remain 0. If enabling `/v1/responses` or `/v1/chat/completions` would be required, STOP at that real config/runtime gate. |
...
| **WINDOWS OPENCLAW** | fallback-only · `2026.5.20` · `127.0.0.1:18789` · Tailscale Serve `https://asusdesktop.tailc01234.ts.net/` · auth mode `token` · no Funnel/public exposure |
...
- **D-0016-W is now the active independent workstream.** Its first phase is read-only and must not issue a planner/provider inference request.
...
- Metadata/auth validation such as `GET /v1/models` is allowed only if the compatible HTTP surface is already enabled; do not infer-enable it.
- Enabling `/v1/responses` or `/v1/chat/completions` is a separate config/runtime gate.
