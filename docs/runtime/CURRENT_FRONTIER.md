...
| **ACTIVE WORK** | WF40 V4 lanes **APPLIED LIVE (83 nodes)** · Windows execution endpoint **PERSISTED** · production PostgreSQL 16.15 **LIVE** · six-profile Qwen **Control Plane eligibility set PASS on 9-profile router superset** · AGG runtime role correction **PASS** (FAST_AGENT unqualified) · workstation-only OpenCode 64K profile **LIVE-VERIFIED / PASS** · workstation-only OpenCode 24K DEV profile **LIVE-VERIFIED / PASS (placement remediation)** · **LOCAL_DEV_EXECUTOR FIRST COMPLETE REAL QWEN EXECUTION = PASS** · no ACTIVE authorization · **#60 VPS migration · #64 historical 16GB replica PASS (host cancelled) · #67 replacement 8GB NEW active** (no cutover) |
...
| **VPS MIGRATION** | #67 **active replacement NEW** `ionos-n8n-new` = `31.70.139.73` (8GB class: 6 vCPU / 7.7 GiB / 232G) · baseline provisioning **PASS** (`V4_REPLACEMENT_8GB_VPS_BASELINE_PROVISIONING_V1`) · Docker 29.8.0 + Compose v5.5.1 · Node 22.23.2 · Tailscale NeedsLogin · swap 4GiB · users 1000/1001 · zero app containers · prior 16GB target `31.70.139.51` **cancelled/historical** (#62–#64 evidence retained) · OLD `ionos-n8n` LIVE · report `reports/architecture/v4_replacement_8gb_vps_baseline_provisioning_v1.md` |
| **NEXT** | **PARALLEL:** (1) always-on LEFT ACTIVE on OLD — idle ticks `IDLE_CLEAN`; (2) **#67 next** = isolated core replica (PostgreSQL/n8n/LiteLLM) on 8GB NEW using proven #64 `unpublish:workflow --all` before n8n start · Tailscale/hostname-MagicDNS/DNS/public-route/cutover stay **GATE** · Hermes later within #67 · `GPT_WEB_N8N_AUTHORING_REQUIRED` gate SUPERSEDED by exercised GPT_WEB override |
