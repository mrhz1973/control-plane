...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · F01 nginx include PASS · nginx private `:443` functional PASS (not enabled) · F02 GIS endpoints NEW-retargeted (GIS still inactive) · ORS loopback+CORS NEW GIS origin PASS (active/not enabled) |
| **VPS MIGRATION** | #68 GraphHopper/ORS/nginx private-chain functional PASS · F01 `EFFECTIVELY_INCLUDED_SYNTAX_VALID` · F02 `NEW_IDENTITY_RETARGETED` · GIS/Nav/D-Flight inactive · TLS issuance PASS · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | nginx HTTPS→ORS chain PASS · report `reports/architecture/v4_vps_goi_nginx_private_chain_functional_qualification_v1.md` |
| **NEXT** | Remaining private GOI runtime slices (GIS/Nav/D-Flight as separately authorized) · schema-engine qualification · restart-persistence/parallel validation · human cutover GATE · OpenClaw runtime `KEEP_STAGED_PENDING` |
