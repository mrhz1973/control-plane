...
Ultimo aggiornamento: 2026-05-31 — Wf Telegram inbound polling/getUpdates live PASS
...
- Prossimo gate reale: harden Wf polling path for safe repeated use — state/offset persistence, allowed-chat guard without raw chat_id in Git, duplicate handling, and optional registration of a future button UI/send step. Any persistent schedule, production Data Table, deleteWebhook, credential handling, or operational automation remains a separate runtime/security gate; perform one step at a time.
