...
| **MICRO_TASK_DELTA LAW** | `docs/foundation/MICRO_TASK_DELTA_OPERATING_LAW.md` — **CANONICAL** (#42) · **ADMISSION+SAFE-FF LIVE** (#48/#50) · **#54 S11 harness fix PASS** (`f3ef3c0`) · CRASH RECOVERY #55/#51 STOP (D-9301-E E4; D-9301-F `STOP:UNEXPECTED_FILE_CHANGES` fake-envelope artifact) · **#56 PERSISTENCE ISOLATION PASS** (`68ce67b` pushed/verified — ONE boolean `realRuntimePersistence` guards BOTH envelope+receipts writes; injected tests persist neither; S13 regression 13/13; dispatcher live-reloaded PID 51048, `405 POST_ONLY`) · **D-9301-G STOP (deterministic)** — natural tick claimed once (07:00:40Z), admission PASS, OpenCode executed once (session `ses_f8a7af44…`), suite 13/13 green, but ZERO edits (S13 already present from #56) → empty stageable set → `STOP:GIT_PERSISTENCE_FAILED (NOTHING_STAGEABLE_IN_SCOPE)`; controller sequencing conflict, not an executor defect; **3-task campaign STOPPED before #52/#53** |
