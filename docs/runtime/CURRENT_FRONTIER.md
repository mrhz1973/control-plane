...
| **ACTIVE WORK** | issue **#25** `D-0019-W` — deterministic OpenClaw request builder; issue **#24 D-0018-W** COMPLETE; issue **#23 D-0017-W** COMPLETE; issue **#22 D-0016-W** Phase B authorized but HOME host offline; issue **#8** Z.AI VPS/provider support parallel |
| **BLOCCO ATTIVO** | `OPENCLAW-REQUEST-BUILDER-REPO-ONLY` |
| **STATO BLOCCO** | `D0019_W_REPO_ONLY_IMPLEMENTATION_READY / D0018_W_COMPLETE / D0016_W_PHASE_B_AUTHORIZED_HOME_HOST_OFFLINE / ZAI_SUPPORT_WAIT_PARALLEL` |
| **GATE CORRENTE** | `D0019_W_REPO_ONLY_AUTO_ELIGIBLE` — bounded local implementation; no runtime/provider/credential gate is opened by this task |
| **NEXT** | Cursor on WORK PC syncs to origin/main and implements issue #25: validate `consumer_input`, deterministically build the exact `/v1/responses` request envelope, source the function parameters from the canonical Execution Packet schema, run local tests, persist evidence. No network/provider/runtime calls. |
...
| **D-0019-W REQUEST BUILDER** | ACTIVE repo-only · issue #25 · input-schema validation + deterministic non-secret `/v1/responses` request envelope · no HTTP execution |
...
- **D-0019-W is active and repo-only:** build a deterministic request envelope from a validated `consumer_input`; use `docs/contracts/openclaw-consumer-input-v1.schema.json`; source `emit_execution_packet.parameters` from `docs/contracts/execution-packet-v1.schema.json`; never place bearer-token values in generated output.
...
- Active deterministic request builder: issue **#25** (`D-0019-W`)
