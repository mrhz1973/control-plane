...
| **ACTIVE WORK** | issue **#24** `D-0018-W` **COMPLETE**; issue **#23 D-0017-W** COMPLETE; issue **#22 D-0016-W** Phase B authorized but HOME host offline; issue **#8** Z.AI VPS/provider support parallel |
| **BLOCCO ATTIVO** | none for D-0018-W |
| **STATO BLOCCO** | `D0018_W_COMPLETE / D0017_W_VALIDATOR_PASS / D0016_W_PHASE_B_AUTHORIZED_HOME_HOST_OFFLINE / ZAI_SUPPORT_WAIT_PARALLEL` |
| **GATE CORRENTE** | `D0018_W_COMPLETE` — GPT-Web hard-constraints mapping incorporated; response gate enforces exact deep-array equality with `HARD_CONSTRAINT_MISMATCH`; D-0017 5/5 and D-0018 15/15 PASS |
| **NEXT** | HOME Phase B remains separately authorized/not executed while host is offline. No further D-0018 work. Provider-call Phase C remains ungated until explicit backend planner/model selection. |
...
| **HARD CONSTRAINT MAPPING** | INCORPORATED · `docs/contracts/execution-packet-hard-constraints-mapping-v1.md` · required packet field `hard_constraints: string[]` · exact order-sensitive element-for-element mapping · mismatch classification `HARD_CONSTRAINT_MISMATCH` |
| **EXECUTION PACKET MACHINE SCHEMA** | `docs/contracts/execution-packet-v1.schema.json` · JSON Schema 2020-12 · required `hard_constraints` array<string> |
| **D-0017-W VALIDATOR** | COMPLETE · fixtures updated for `hard_constraints` · `tools/validate-execution-packet-v1.mjs` · local tests 5/5 PASS |
| **D-0018-W RESPONSE GATE** | COMPLETE · `tools/validate-openclaw-planner-response-gate.mjs` · exact deep-array equality · local tests 15/15 PASS · `HARD_CONSTRAINT_MAPPING_UNDEFINED` removed as operational blocker |
...
- **D-0018-W COMPLETE:** GPT-Web mapping `docs/contracts/execution-packet-hard-constraints-mapping-v1.md` is incorporated into packet contract, machine schema, OpenClaw consumer contract, D-0017 fixtures, and D-0018 gate. Exact deep-array equality; mismatch → `HARD_CONSTRAINT_MISMATCH`.
...
- Completed mapping integration: issue **#24** (`D-0018-W`)
...
- Completed packet-validator: issue **#23** (`D-0017-W`) · `tools/validate-execution-packet-v1.mjs` · `tests/execution-packet-validator/`
