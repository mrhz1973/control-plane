...
Ultimo aggiornamento: 2026-05-31 — Wf Telegram inbound polling/getUpdates package prepared
...
- Prossimo gate reale: Wf live — import workflow 47, configure Telegram token/credential and allowed chat/state placeholders in n8n UI only, run one manual polling/getUpdates test, verify sanitized decision receipt, then return evidence for registration. Any deleteWebhook, persistent polling schedule, production Data Table, credential/token handling, or inbound activation is a separate runtime/security gate; perform one step at a time.
