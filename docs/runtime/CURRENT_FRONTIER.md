...
Ultimo aggiornamento: 2026-05-31 — Wf47 Wg operationalization checklist PREP PASS (no-runtime)
...
- Wf47->Wg operationalization checklist: PREP PASS — increment 2 no-runtime config checklist in docs/workflow-wf47-wg-operationalization-checklist.md; confirms docs/templates/CSV policy/boundaries before n8n import rehearsal; no runtime; workflows/data-tables untouched by task; PM-34 BLOCCATO.

## Frontiera attuale / prossimo gate
- Prossimo gate reale: Wf47->Wg operationalization increment 3 — manual inactive/off import rehearsal of wf47/wg/wh in n8n UI (user-attested PASS registration), one gate at a time. Increment 2 checklist PREP PASS complete. Optional prior increment 1: Wh scenarios note_only/malformed/stale_closed if needed. No schedule. No production Data Table. No PM-34 without explicit separate gate. Wf47, Wg, and Wh manual validations remain PASS. Telegram inbound operational automation remains NOT RUN / NOT ACTIVE.
