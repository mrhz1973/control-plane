...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · remaining GOI (ORS/GIS/Nav/D-Flight/nginx) activation remains GATE |
| **VPS MIGRATION** | #68 GraphHopper functional qualification **PASS** · `GOI_GRAPHHOPPER_FUNCTIONAL_QUALIFICATION=PASS` · boot persistence **PENDING** · TLS issuance still PASS · nginx/ORS/GIS/Nav/D-Flight inactive · report `reports/architecture/v4_vps_goi_graphhopper_activation_v1.md` · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | `CODEX_VPS_EVIDENCE_AUDIT=BLOCKED_EVIDENCE` · effective nginx vhost inclusion and GIS NEW endpoint transition not proven in repository evidence · existing GraphHopper functional/TLS identity PASS preserved · report `reports/architecture/v4_vps_codex_independent_evidence_audit_v1.md` |
| **NEXT** | Cursor bounded read-only evidence: NEW nginx effective include/test target + GIS effective GraphHopper/ORS endpoints → close independent evidence audit before generic remaining-GOI activation clearance → remaining private slices → schema-engine qualification → restart-persistence/parallel validation · human cutover GATE · OpenClaw runtime `KEEP_STAGED_PENDING`, final disposition later |
