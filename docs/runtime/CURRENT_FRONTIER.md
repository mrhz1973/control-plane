...
Ultimo aggiornamento: 2026-05-31 — Wf47 ready-import Data Table template prep
...
- Wf hardened path manual validation (staticData template): PARTIAL/BLOCKED — first poll accepted D-9998-T option 1; second poll re-accepted same update_id; duplicate/stale NOT validated on staticData path. Safe repeated use remains blocked on that path.
- Prossimo gate reale: Wf47 Data Table ready-import manual validation — delete/avoid duplicate workflow 47 in n8n; import ready template once; configure token (HTTP) and allowed_chat_id (Set node) in UI only; confirm wf47_polling_state_test seeded; run manual poll 1 (expect accepted) + poll 2 without new message (expect duplicate_or_stale true, not accepted). One step only. No schedule. No PM-34. Not production control_plane_state unless separately gated.
