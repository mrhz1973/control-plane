...
| **VPS MIGRATION** | #68 prep-copy **PASS** · report `reports/architecture/v4_replacement_8gb_full_service_parity_prep_copy_v1.md` · NEW has GOI/OpenClaw/handoff trees + disabled units · nginx inactive · Tailscale **NeedsLogin** · MISSING residual = Tailscale unique hostname/join + live GOI/TLS activation · Hermes #67 web/auth/persistence/recall/GREEN reconciled · OLD LIVE |
| **NEXT** | Tailscale unique hostname/join on NEW → MagicDNS/TLS → enable+qualify GOI stack · parallel validation · human cutover GATE · OpenClaw activate-or-archive human decision |
