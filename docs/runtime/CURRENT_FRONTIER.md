...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · nginx/GIS activation withheld until F01 include + F02 GIS endpoint remediation |
...
| **VPS INDEPENDENT EVIDENCE AUDIT** | F01/F02 **evidence complete** · F01=`STAGED_NOT_INCLUDED` · F02=`ACTIVE_OLD_ENDPOINT_FOUND` · `CODEX_VPS_EVIDENCE_AUDIT` configuration gaps remain (not evidence gaps) · report `reports/architecture/v4_vps_goi_f01_f02_readonly_evidence_v1.md` |
| **NEXT** | Separate remediation deltas for nginx GOI vhost include (F01) and GIS HTML GraphHopper/ORS NEW retarget (F02) · then remaining private GOI slices · schema-engine qualification · restart-persistence/parallel validation · human cutover GATE · OpenClaw runtime `KEEP_STAGED_PENDING` |
