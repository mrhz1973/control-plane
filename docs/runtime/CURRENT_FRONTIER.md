...
| **ACTIVE WORK** | WF40 V4 lanes **APPLIED LIVE (83 nodes)** · Windows execution endpoint **PERSISTED** · production PostgreSQL 16.15 **LIVE** · six-profile Qwen **Control Plane eligibility set PASS on 9-profile router superset** · AGG runtime role correction **PASS** (FAST_AGENT unqualified) · workstation-only OpenCode 64K profile **LIVE-VERIFIED / PASS** · workstation-only OpenCode 24K DEV profile **LIVE-VERIFIED / PASS (placement remediation)** · **LOCAL_DEV_EXECUTOR FIRST COMPLETE REAL QWEN EXECUTION = PASS** · no ACTIVE authorization · **#60/#62 inventory PASS · #63 STOP (active=false insufficient) · #64 NEW localhost replica isolated PASS** (no cutover) |
...
| **VPS MIGRATION** | #60/#62 inventory **PASS** · #63 STOP (`active=false` ignored published `activeVersionId`) · #64 `V4_NEW_VPS_N8N233_DISABLE_BEFORE_START_AND_LOCALHOST_REPLICA_RETRY_V1` **PASS** · NEW loopback n8n 2.33.3 unpublished (`unpublish:workflow --all`) + PG replica from dump + unpublished LiteLLM · OLD `ionos-n8n` LIVE unchanged · Tailscale NEW Logged out · report `reports/architecture/v4_new_vps_n8n233_disable_before_start_and_localhost_replica_retry_v1.md` |
| **NEXT** | **PARALLEL:** (1) always-on LEFT ACTIVE on OLD — idle ticks `IDLE_CLEAN`; (2) remaining #60 slices (Tailscale join / hostname-MagicDNS / DNS / public route / cutover) stay **GATE** · NEW replica must remain unpublished until an authorized cutover task · `GPT_WEB_N8N_AUTHORING_REQUIRED` gate SUPERSEDED by exercised GPT_WEB override |
