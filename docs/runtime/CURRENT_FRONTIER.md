...
| **ACTIVE WORK** | WF40 V4 routing + sidecar-source + local RESOURCE_STATUS + execution-adapter-router lanes **APPLIED LIVE (66 nodes)** · n8n adapter-router bridge **OFFLINE / LIVE-INCAPABLE** · Windows-local execution endpoint contract **GPT-WEB AUTHORED / NOT IMPLEMENTED** |
| **BLOCCO ATTIVO** | `V4_WINDOWS_LOCAL_EXECUTION_ENDPOINT_OFFLINE_IMPLEMENTATION` |
| **STATO BLOCCO** | RUNNER_TRANSPORT_DISCOVERY_PASS / EXECUTION_ENDPOINT_CONTRACT_AUTHORED / REQUEST_RESPONSE_SCHEMAS_AUTHORED / OFFLINE_IMPLEMENTATION_PENDING / WF40_66_UNCHANGED / LIVE_RUNNER_ABSENT / WF61_INACTIVE / D0025_CLOSED |
| **GATE CORRENTE** | **CLOSED TO LIVE EXECUTION** · D-0025 closed · no Qwen/OpenCode/provider generation authorized · GPT-Web has authored the Windows-local execution endpoint contract; next block is offline implementation + DI tests only, with zero runtime/service/Tailscale/workflow mutation |
| **NEXT** | `V4_WINDOWS_LOCAL_EXECUTION_ENDPOINT_OFFLINE_IMPLEMENTATION` — implement the Windows-local execution endpoint tool + request/response validation/tests against `docs/contracts/v4-windows-local-execution-endpoint-v1.md`. Offline/DI only: no service install/start, no Tailscale mutation, no WF40 mutation, no live OpenCode/Qwen/provider execution. |
...
| **WINDOWS EXECUTION ENDPOINT CONTRACT** | `docs/contracts/v4-windows-local-execution-endpoint-v1.md` · GPT-Web authored · POST-only target `/v4/execution/opencode-local` → future loopback `127.0.0.1:18791` · implementation not yet present |
...
## Authored Windows-local execution endpoint target

```text
VPS / n8n
  -> Tailscale-private HTTPS POST /v4/execution/opencode-local
  -> future Windows loopback service 127.0.0.1:18791
  ...
  -> future fixed no-shell OpenCode runner
...
- execution endpoint implementation must be offline/DI only in the next block;
- no service persistence/Tailscale Serve/firewall mutation in the next block;
