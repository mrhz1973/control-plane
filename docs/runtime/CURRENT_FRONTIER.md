...
Ultimo aggiornamento: 2026-05-31 — Wg inbound decision state correlation package prepared
...
- Prossimo gate reale: Wg manual validation — create/seed test-only Data Table wg_decision_state_test (open row D-9998-T), import inactive workflow 48, run scenarios valid_close (expect closed), duplicate (expect duplicate_or_already_closed), unknown (expect unknown_decision_id); record sanitized Inspect correlation receipts. One step at a time. No schedule. No production control_plane_state. No PM-34. Wf47 Data Table manual validation remains PASS. Telegram inbound operational automation remains NOT RUN / NOT ACTIVE.
