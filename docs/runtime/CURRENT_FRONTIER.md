...
| **NEXT** | `V4_LOCAL_DEV_EXECUTOR_BACKLOG_ENVELOPE_BRIDGE_IMPLEMENT_V1` — implementazione del ponte backlog→local-dev-task-envelope-v1 esattamente secondo il design persistito (`reports/architecture/v4_local_dev_executor_backlog_envelope_bridge_design_v1.md`): tool `tools/bridge-backlog-to-local-dev-envelope-v1.mjs` riusando extractYamlFence/parseBoundedBacklogYaml + validateEnvelope invariati, fixtures offline, gate fail-closed DEV-lane (state/human-gate/high-risk/unknown local_dev keys), receipt idempotency v1. Nota campagna: V4_LOCAL_DEV_EXECUTOR_OVERNIGHT_AUTOVIA_CAMPAIGN_V1 in corso (autorizzazione operatore notturna, lane DEV only) |
