...
| **MICRO_TASK_DELTA LAW** | `docs/foundation/MICRO_TASK_DELTA_OPERATING_LAW.md` — **CANONICAL** (`V4_TOKEN_EFFICIENCY_MICRO_TASK_POLICY_PERSISTENCE_V1`, #42): DEFAULT TASK UNIT=`MICRO_TASK_DELTA`; `max_corrective_loops=2`; focused tests only; broad regression/BugBot = `CHECKPOINT_DELTA` only; `HUMAN_AUTHORIZED_CAMPAIGN_EXCEPTION` preserved; `AUTOMATION_PARITY` persisted (no live automation change); consumers: CURSOR_PROMPT_TEMPLATE / USER_HANDOFF / PROMPT_SEQUENCING_GATE / MULTI_PLANNER; focused lint **11/11** · **ADMISSION PARITY** (`V4_AUTOMATED_MICRO_TASK_ADMISSION_PARITY_V1`, #47): `tools/admit-micro-task-delta-v1.mjs` wired in dispatcher `performTick` BEFORE executor — CODE_WIRED; live process still requires **dispatcher service restart** (dependency persisted; not performed this pass); focused admission **11/11** + dispatcher service **10/10** |
