...
Ultimo aggiornamento: 2026-06-12 — D-0032-W field-validation PASS attestato (manual one-shot).
...
**Not auto-started.** D-0032-W field-validation **closed**. Gate esplicitamente **chiusi / fuori scope** salvo Decision Packet separato: schedule · loop · wrapper HTTP · callable-from-n8n worker · PM-34 unlock · `n8n_ready=true`. Recommended next: hardening/cleanup **`D-9999-T`** + **47/Wf** target, **or** Gate C (We/46 HTTPS) / Option 4 — **separate explicit decision** only. Boundaries: NO PM-34 · NO `n8n_ready=true` · NO permanent loop · wf40/42 untouched · pezzi collegati ≠ loop avviato.
