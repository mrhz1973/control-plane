...
| **MICRO_TASK_DELTA LAW** | `docs/foundation/MICRO_TASK_DELTA_OPERATING_LAW.md` — **CANONICAL** (#42) · **ADMISSION PARITY LIVE** (#48, PID was 55972 at apply; may change after later restarts) · **SAFE FF SYNC CODE_WIRED** (`V4_DISPATCHER_SAFE_FAST_FORWARD_SYNC_V1`, #49, base `0850b21`): dispatcher `verifyRepoState` may `git merge --ff-only origin/main` when clean+behind; ahead/diverged/dirty/ff-fail → HUMAN_GATE; **live restart still required** to load FF sync (not performed in #49); focused ff-sync **9/9** |
