...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · F01/F02 PASS · GraphHopper/ORS/nginx/D-Flight/GIS/Nav private functional PASS (not enabled) |
| **VPS MIGRATION** | #68 GraphHopper/ORS/nginx/D-Flight/GIS/Nav private functional PASS (not enabled) · schema-engine `MIGRATED_VALIDATED` · F01/F02 PASS · TLS issuance PASS · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | schema-engine NEW functional PASS · report `reports/architecture/v4_vps_schema_engine_new_functional_qualification_v1.md` |
| **NEXT** | GOI boot/restart persistence + active-nginx TLS renewal (separately authorized) · parallel validation · human cutover GATE · OpenClaw runtime `KEEP_STAGED_PENDING` |
