...
| **MICRO_TASK_DELTA LAW** | `docs/foundation/MICRO_TASK_DELTA_OPERATING_LAW.md` — **CANONICAL** (#42) · **ADMISSION+SAFE-FF LIVE** (#48/#50) · **#54/#56 PASS** · **D-9301-H LIVE PASS** (`1fc0189`) · historical E/F/G STOP preserved · **#52 CHAIN STOP** — D-9302-A authored S15 (OpenCode once) but no executor-pass; S4 `tickDeps:{verifyRepo only}` spuriously claimed D-9302-B mid-suite with fake head `aaa…` and wrote real B envelope/receipt; A's persist fail-closed; S15 restored as crash-recovery residue (not executor-pass); **campaign `V4_FRESH_H_AND_TWO_TASK_CHAIN_FINAL_CLOSURE_V1` STOPPED before #53**; required future fix: S4 must fully inject scan/dispatch/executor (or persistence gate must treat any injected verifyRepo as non-persistent) |
