...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · F01 nginx include PASS · nginx private `:443` functional PASS (not enabled) · F02 GIS endpoints NEW-retargeted · GIS private `:8000` functional PASS (not enabled) · ORS loopback+CORS NEW GIS origin PASS (active/not enabled) |
| **VPS MIGRATION** | #68 GraphHopper/ORS/nginx/D-Flight/GIS private functional PASS (not enabled) · F01/F02 PASS · Nav inactive · TLS issuance PASS · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | GIS private functional PASS · report `reports/architecture/v4_vps_goi_gis_private_functional_qualification_v1.md` |
| **NEXT** | Remaining private GOI runtime slice (Nav as separately authorized) · schema-engine qualification · restart-persistence/parallel validation · human cutover GATE · OpenClaw runtime `KEEP_STAGED_PENDING` |
