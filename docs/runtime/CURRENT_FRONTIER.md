...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · F01 nginx include PASS (nginx still inactive) · F02 GIS endpoints NEW-retargeted (GIS still inactive) · ORS loopback+CORS NEW GIS origin PASS (active/not enabled) |
| **VPS MIGRATION** | #68 GraphHopper functional PASS · ORS loopback+CORS PASS · F01 `EFFECTIVELY_INCLUDED_SYNTAX_VALID` · F02 `NEW_IDENTITY_RETARGETED` · `F01_F02_CONFIGURATION_GAPS=CLOSED` · nginx/GIS/Nav/D-Flight inactive · TLS issuance PASS · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | F01/F02 closed · ORS `127.0.0.1:8020` functional PASS · CORS GIS origin NEW · report `reports/architecture/v4_vps_goi_ors_cors_new_gis_remediation_v1.md` |
