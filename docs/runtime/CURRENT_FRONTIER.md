...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · GOI activation remains GATE |
| **VPS MIGRATION** | #68 NEW TLS recovery **PASS** · `NEW_TAILSCALE_TLS_ISSUANCE_QUALIFICATION=PASS` · renewal helper handles inactive nginx correctly · NEW cert/key/SAN/perms/nginx syntax qualified · GOI/nginx remain inactive · report `reports/architecture/v4_vps_new_tls_recovery_v1.md` · OLD LIVE |
| **NEXT** | Controlled GOI activation/qualification on NEW → schema-engine qualification → parallel validation · human cutover GATE · OpenClaw activate-or-archive human decision |
