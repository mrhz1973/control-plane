...
| **ACTIVE WORK** | WF40 V4 lanes **APPLIED LIVE (83 nodes)** · Windows execution endpoint **PERSISTED** · production PostgreSQL 16.15 **LIVE** · six-profile Qwen **Control Plane eligibility set PASS on 9-profile router superset** · AGG runtime role correction **PASS** (FAST_AGENT unqualified) · workstation-only Cline 64K profile **LIVE-VERIFIED / PASS** · workstation-only Cline 24K DEV profile **LIVE-VERIFIED / PASS (placement remediation)** · **LOCAL_DEV_EXECUTOR FIRST COMPLETE REAL QWEN EXECUTION = PASS** · no ACTIVE authorization |
| **BLOCCO ATTIVO** | `V4_LOCAL_DEV_EXECUTOR_BRIDGED_PROOF_TRACKED_FILE_SEMANTICS_V1` — **IMPLEMENTATION PASS / LIVE-PROOF STOP** · (a) NEW_FILE_PERSISTENCE_SEMANTICS implementate in `tools/local-dev-executor-v1.mjs`: snapshot untracked pre-run → classificazione `PREEXISTING_UNTRACKED` (protetti assoluti) vs `TASK_CREATED_UNTRACKED` (stageable solo se in-scope, nuovi, creati nel run, senza violazioni) + staging selettivo esplicito fail-closed (`STOP:PREEXISTING_UNTRACKED_MODIFIED`, `STOP:PATH_NORMALIZATION_AMBIGUOUS`, `STOP:UNEXPECTED_FILE_CHANGES`) · 15 regressioni deterministiche (`tests/local-dev-executor-new-file-persistence-v1`) · suite verdi: 21/21 + 42/42 + 14/14 + 18/18 · (b) bridged live proof retry ESATTAMENTE UNO su `qwen38-opus-q3-cline-24k`: **STOP:OPENCODE_RUN_FAILED** a 8/8 turni (419s/600s) — convergence failure nel layer agente OpenCode (read file inesistente non recuperata + bash negato da permission rule OpenCode); router/executor/bridge/guard sani; 33 pre-existing untracked intatti; tracked clean; NO second retry · canonical STOP: `reports/runtime/cursor-stops/2026-09-05T032418Z__BRIDGED_PROOF_TRACKED_FILE_SEMANTICS_V1.stop.json` |
...
| **NEXT** | **OPERATOR-GATED** — `V4_LOCAL_DEV_EXECUTOR_OPENCODE_PERMISSION_AND_NEW_FILE_CONVERGENCE_REMEDIATION_V1` (proposto): il layer agente OpenCode non completa la creazione di un nuovo file (permission rule blocca bash; read-missing non recuperata) — servono: indagine permission config OpenCode DEV, shaping obiettivo esplicito "create file if absent", eventuale widening permission limitato al lane DEV; oppure decisione operatore di ritorno a obiettivi tracked-file-only. Non auto-eligible: confine agente/permission = scelta strategica |
| **CAMPAIGN** | `V4_LOCAL_DEV_EXECUTOR_OVERNIGHT_AUTOVIA_CAMPAIGN_V1` TERMINATA (STOP sostanziale, regola campagna) · **RIATTIVATA su autorizzazione esplicita operatore** nel dispatch `..._TRACKED_FILE_SEMANTICS_V1` (budget ~5h da quel dispatch) · pass semantica persistenza = PASS (commit `bdcce7b`, remote-verified) · bridged live proof retry ESATTAMENTE UNO = **STOP:OPENCODE_RUN_FAILED** (convergenza agente, non infrastruttura) → **CAMPAIGN TERMINATED di nuovo su STOP sostanziale non coperto da delegated auto-repair** · EXECUTOR_END_HEAD pass = `bdcce7b` (nessun executor commit; STOP) · bookkeeping: EXECUTOR_END_HEAD vs CAMPAIGN_FINAL_HEAD ora distinti nel checkpoint (`..._OVERNIGHT_AUTOVIA_CAMPAIGN_V1.md`) |
...
| **QWEN MODEL POLICY** | `configs/resources/qwen-local-model-policy.json` + `configs/resources/qwen-router-catalog-scope-overlay.json` + `configs/resources/qwen-local-runtime.json` · **6 Control Plane-eligible / 9 router-visible** · startup `qwen38-opus-q3-daily-16k` unchanged · 2 workstation-local 96K profiles OUT OF SCOPE · Cline 64K + Cline 24K under `workstation_manual_profiles`, non-eligible/non-routed; Cline 64K **LIVE-VERIFIED / PASS**; Cline 24K **LIVE-VERIFIED (placement remediation smoke)** · next WF40 executor `qwen38-opus-q3-agent-24k` unchanged |
