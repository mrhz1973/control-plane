...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · F01 nginx include PASS (nginx still inactive) · GIS activation withheld until F02 endpoint remediation |
| **VPS MIGRATION** | #68 GraphHopper functional qualification **PASS** · F01 nginx vhost **EFFECTIVELY_INCLUDED_SYNTAX_VALID** · nginx runtime inactive · F02 GIS still `ACTIVE_OLD_ENDPOINT_FOUND` · TLS issuance PASS · ORS/GIS/Nav/D-Flight inactive · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | F01 evidence+include remediation **PASS** · F02=`ACTIVE_OLD_ENDPOINT_FOUND` remains · report `reports/architecture/v4_vps_goi_f01_nginx_include_remediation_v1.md` |
| **NEXT** | F02 GIS HTML GraphHopper/ORS NEW retarget (separate delta) · then remaining private GOI slices · schema-engine qualification · restart-persistence/parallel validation · human cutover GATE · OpenClaw runtime `KEEP_STAGED_PENDING` |
