...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · F01/F02 PASS · GOI stack active/**enabled** after cold-start persistence PASS · active-nginx TLS renewal pending |
| **VPS MIGRATION** | #68 GOI stack functional+cold-start persistence PASS (enabled, no reboot proof needed structurally) · schema-engine `MIGRATED_VALIDATED` · F01/F02 PASS · TLS issuance PASS · active-nginx renewal pending · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | GOI cold-start boot persistence PASS · report `reports/architecture/v4_vps_goi_cold_start_boot_persistence_v1.md` |
| **NEXT** | active-nginx TLS renewal qualification (separately authorized) · parallel OLD↔NEW validation · human cutover GATE · OpenClaw runtime `KEEP_STAGED_PENDING` |
