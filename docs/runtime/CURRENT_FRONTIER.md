...
| **GATE CORRENTE** | **CLOSED** · D-0025 `enabled=false` · VPS cutover **CLOSED** · NEW Tailscale unique identity joined (`100.99.54.93`, `ionos-n8n-new.tailc01234.ts.net`) · F01/F02 PASS · GOI stack active/**enabled** + cold-start persistence PASS · active-nginx TLS renewal PASS (timer enabled weekly) |
| **VPS MIGRATION** | #68 GOI stack functional+cold-start persistence+active-nginx TLS renewal PASS · schema-engine `MIGRATED_VALIDATED` · F01/F02 PASS · OLD LIVE |
| **VPS INDEPENDENT EVIDENCE AUDIT** | parallel OLD↔NEW validation PASS · F03 reconciled · F04 OLD `:80` non-required obsolete default · F05 current OLD HTTPS PASS with renewal degraded · report `reports/architecture/v4_vps_parallel_old_new_validation_f03_f04_f05_v1.md` |
| **NEXT** | HUMAN CUTOVER GATE: final OLD write freeze + proven DB/state sync + fresh publication map + authorized NEW publication/routing · OLD rollback retention; OpenClaw runtime remains `KEEP_STAGED_PENDING` |
