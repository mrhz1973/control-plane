...
| **STATO BLOCCO** | `HUMAN_GATE_REQUIRED / ENDPOINT_HOST_REMEDIATION` |
| **GATE CORRENTE** | `GLM_BIGMODEL_CN_CODING_ENDPOINT_REMEDIATION_GATE_REQUIRED` |
| **NEXT** | compare the working Cursor configuration using `https://open.bigmodel.cn/` with current OpenClaw `https://api.z.ai/api/coding/paas/v4`; if explicitly authorized, change only the Z.AI baseUrl host/path to the official BigModel CN Coding endpoint `https://open.bigmodel.cn/api/coding/paas/v4`, preserve the existing credential/profile, perform local postchecks only, then require a separate smoke gate |
...
| **REGIONAL HOST HYPOTHESIS** | strong: current VPS uses global `api.z.ai` host while the operator's working Cursor configuration and current official BigModel CN docs use `open.bigmodel.cn`; prior provider/account-plan blocker is superseded pending bounded host remediation |
...
| **PLANNER SMOKE** | Codex PASS · GLM blocked on global host pending bounded BigModel CN endpoint remediation · Qwen 3.8 37B blocked missing model |
...
- No runtime mutation has been performed from this new evidence. The next mutation, if explicitly authorized, is limited to `models.providers.zai.baseUrl` from the global host to `https://open.bigmodel.cn/api/coding/paas/v4`; existing `zai:manual` credential/profile must be preserved and no model/provider request is allowed in the remediation pass.
- A successful local endpoint remediation would NOT prove provider functionality. Any subsequent GLM smoke requires a separate human gate and exactly one invocation.
...
- No credential refresh/re-entry, retry, alternate GLM smoke, Codex/Qwen invocation, core/plugin upgrade, doctor --fix, gateway/service activation, n8n/Docker/Tailscale mutation, firewall/reverse proxy, runtime wiring or billing is authorized before the endpoint-host remediation gate.
