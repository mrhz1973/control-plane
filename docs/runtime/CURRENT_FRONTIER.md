...
| **ACTIVE WORK** | WF40 V4 lanes **APPLIED LIVE (83 nodes)** · Windows execution endpoint **PERSISTED** · production PostgreSQL 16.15 **LIVE** · six-profile Qwen **Control Plane eligibility set PASS on 9-profile router superset** · AGG runtime role correction **PASS** (FAST_AGENT unqualified) · workstation-only OpenCode 64K profile **LIVE-VERIFIED / PASS** · workstation-only OpenCode 24K DEV profile **LIVE-VERIFIED / PASS (placement remediation)** · **LOCAL_DEV_EXECUTOR FIRST COMPLETE REAL QWEN EXECUTION = PASS** · no ACTIVE authorization · **#60 VPS migration · #67 core+Hermes non-auth PASS · #68 full service parity census PASS** (`OLD_DECOMMISSION_ELIGIBLE=NO`) |
...
| **VPS MIGRATION** | #68 census **PASS** · report `reports/architecture/v4_replacement_8gb_full_service_parity_census_v1.md` · NEW `31.70.139.73` has core+Hermes non-auth only (9 MIGRATED_VALIDATED) · **17 MISSING** (Tailscale/nginx/GOI GraphHopper/ORS/D-Flight/GIS/Nav + handoff repos + `n8n-compose` boot) · **5 PRESENT_NOT_VALIDATED** · **3 OBSOLETE_NEEDS_HUMAN_DECISION** · `OLD_DECOMMISSION_ELIGIBLE=NO` · OLD `217.160.71.145` LIVE |
| **NEXT** | **#68 migrate backlog** from census MISSING dependency order (Tailscale unique hostname first) · Hermes ChatGPT login still human · Tailscale/DNS/nginx/cutover remain **GATE** · always-on LEFT ACTIVE on OLD |
