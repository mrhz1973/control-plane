...
| **BLOCCO ATTIVO** | `V4_CLINE64K_ROUTER_LATENCY_DETERMINISM_DIAGNOSTIC_V1` — **PASS (classification C)** · due micro-generazioni dirette sul router: 98s e 122s per 32 token (~3-4 s/token, nessun warm-up) → **ROUTER_BASELINE_SLOW_BOTH**: il regime slow esiste al baseline router/modello, non solo a livello OpenCode · RETRY10-12 = historical diagnostics (cold-start / confounded / controlled repro) · gap residuo vs RETRY9 (~30 s/gen) da localizzare lato backend |
...
| **NEXT** | `V4_CLINE64K_BACKEND_RUNTIME_STATE_INSPECTION_V1` — ispezione read-only dello stato del backend llama-server per l'istanza 64K (layer/offload, istanze sibling carite, KV-cache) per spiegare il throughput gap vs condizioni RETRY9, PRIMA di qualsiasi decisione su timebox executor o profilo |
