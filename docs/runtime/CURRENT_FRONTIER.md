...
| **QUOTA_AWARE_RUNTIME** | **RUNTIME_WIRED_BEHIND_CLOSED_GATE** · campaign `V4_RUNTIME_25_TASK_QUOTA_AWARE_CAMPAIGN_V1` (issue #41) 25/25 tasks PASS · chain: ingest codex/glm → composer → quota-state join → freshness/reserve/economics → planner/execution/reviewer/retry selectors → guards (quality/urgency/eligibility) → decision audit JSONL → packet provenance → n8n bridge consumption → Windows endpoint validation → observability visibility · closed-gate E2E **PROVEN** (`tests/rt25-t24-closed-gate-e2e` 23/23: propagation, stale/missing fail-closed, reserve block, shared-pool single-admission, quality veto, admission blocked with 0 adapter calls, D-0025 unchanged) · shared pools: `chatgpt_codex_subscription` (subscription-only), `glm_coding_plan` (5.3+Flash) · GLM live collector **BLOCKED_EVIDENCE** (credential absent → UNKNOWN fail-closed) · no production route activation; bridge `dispatch_prepared=false` invariant intact · closure `reports/architecture/v4_rt25_quota_aware_runtime_wiring_closure_v1.md` |
