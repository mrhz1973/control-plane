...
| **NEXT** | authorize exactly one minimal direct smoke invocation of exact `zai/glm-5.3` through authenticated OpenClaw on VPS; no retry, fallback, gateway/service, n8n wiring, Codex/Qwen invocation or broader runtime activation |
...
| **PLANNER SMOKE** | Codex VPS: PASS · GLM VPS: READY_PENDING_EXPLICIT_ONE_INVOCATION_GATE · Qwen 3.8 37B: BLOCKED_MISSING_MODEL |
| **PM-34** | BLOCKED |
| **n8n_ready** | `false` |
| **Gate E** | PASS / CLOSED |
| **L5_PASS** | NOT_CLAIMED |
...
- Auth Z.AI/profile/provider restano integri; gateway false; port `18789` free; system Node/npm invariati; GLM invocation count `0`.
- Il prossimo gate è separato e umano: autorizzare esattamente una minimal direct smoke invocation dell'esatto `zai/glm-5.3`.
