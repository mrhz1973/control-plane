...
**PM-50 PASS:** OpenClaw install/onboard — [PM50](PM50_OPENCLAW_LOCAL_INSTALL_ONBOARD_PASS.md). **PM-51:** **PASS** (casa, gateway no-op `/health` 200) — [session](sessions/2026-05-22-control-plane-pm51-openclaw-confined-gateway-noop-probe-pass.md) · [PM51](PM51_OPENCLAW_CONFINED_GATEWAY_NOOP_PROBE_GATE.md). **PM-56:** contract review **PASS** (docs-only) — [PM56](PM56_OPENCLAW_ADAPTER_CONTRACT_REVIEW.md); no code changed; next **PM-57** fixture review candidate; raw OpenClaw output **never** direct to n8n; PM-34 **blocked**. **PM-55:** adapter dry-run **PASS** — [PM55](PM55_OPENCLAW_BRIDGE_ADAPTER_DRY_RUN.md); `n8n_ready` **false**. **PM-53:** validator **PASS** — [PM53](PM53_OPENCLAW_BRIDGE_ARTIFACT_VALIDATOR_DRY_RUN.md). **PM-54:** design only — [PM54](PM54_OPENCLAW_BRIDGE_ADAPTER_DESIGN.md). **PM-52:** design only — [PM52](PM52_OPENCLAW_CONFINED_BRIDGE_DESIGN.md). **PM-34:** **blocked**. **Macchine:** PM-51 runtime solo casa (`mrhz`); PC lavoro = docs-only.
