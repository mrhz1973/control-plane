...
**PM-50 PASS:** OpenClaw install/onboard — [PM50](PM50_OPENCLAW_LOCAL_INSTALL_ONBOARD_PASS.md). **PM-51:** **PASS** (casa, gateway no-op `/health` 200) — [session](sessions/2026-05-22-control-plane-pm51-openclaw-confined-gateway-noop-probe-pass.md) · [PM51](PM51_OPENCLAW_CONFINED_GATEWAY_NOOP_PROBE_GATE.md). **PM-54:** adapter design **prepared** — [PM54](PM54_OPENCLAW_BRIDGE_ADAPTER_DESIGN.md); `n8n_ready` **false** until future gate; next **PM-55** adapter dry-run; PM-34 **blocked**. **PM-53:** validator dry-run **PASS** — [PM53](PM53_OPENCLAW_BRIDGE_ARTIFACT_VALIDATOR_DRY_RUN.md). **PM-52:** design only — [PM52](PM52_OPENCLAW_CONFINED_BRIDGE_DESIGN.md). **PM-34:** **blocked**. **Macchine:** PM-51 runtime solo casa (`mrhz`); PC lavoro = docs-only.
