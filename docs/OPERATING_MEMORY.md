...
**PM-21 candidate import:** `workflows/exports/READY_IMPORT_42-classifier-bridge-candidate.json` — name `42 - CP v4 multirepo + classifier bridge - CANDIDATE OFF` · **active false**. Build: `tools/build-ready-import-42-bridge-candidate.mjs` from READY_IMPORT_40. **Never** silently edit published `40`. **PM-21 Code nodes:** `runOnceForEachItem` → use **`$json`**, not `$input.first()` or `$('…').first()`; bridge passes `classifier` on result json.
