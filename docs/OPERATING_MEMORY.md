...
**PM-50 PASS:** OpenClaw install/onboard — [PM50](PM50_OPENCLAW_LOCAL_INSTALL_ONBOARD_PASS.md). **PM-51:** **PASS** (casa, gateway no-op `/health` 200) — [session](sessions/2026-05-22-control-plane-pm51-openclaw-confined-gateway-noop-probe-pass.md) · [PM51](PM51_OPENCLAW_CONFINED_GATEWAY_NOOP_PROBE_GATE.md). **PM-53:** bridge artifact validator dry-run **PASS** — `tools/validate-openclaw-bridge-artifact.mjs` · samples `examples/pm53-*`; [PM53](PM53_OPENCLAW_BRIDGE_ARTIFACT_VALIDATOR_DRY_RUN.md); next **PM-54** adapter design; PM-34 **blocked**. **PM-52:** design only — [PM52](PM52_OPENCLAW_CONFINED_BRIDGE_DESIGN.md). **PM-34:** **blocked**. **Macchine:** PM-51 runtime solo casa (`mrhz`); PC lavoro = docs-only.
