...
export const MAX_TTL_MS = 60 * 60 * 1000; // legacy compat cap: 1h (non-authoritative for lifetime_mode="operator_wait")
export const DEFAULT_TTL_MS = 15 * 60 * 1000;
/**
 * HUMAN_WAIT != TOOL_CALL_TIMEOUT (V4_CURSOR_ACP_MCP_HUMAN_GATE_PERSISTENT_OPERATOR_WAIT_V1):
 * a human gate waiting for the operator NEVER auto-expires on wall-clock time.
 * `ttl_ms`/`expires_at` are legacy compatibility fields only — on the
 * operator-wait path they are recorded but NOT enforced. Terminal outcomes
 * are EXPLICIT events only: ANSWERED (canonical admission), CANCELLED,
 * SUPERSEDED (or real unrecoverable failures: TRANSPORT_FATAL/STORE_CORRUPT/
 * BINDING_INVALID). Elapsed time alone is NEVER terminal.
 */
export const OPERATOR_WAIT_MODE = "operator_wait";
 ...
 */
export function registerGateDecision(store, { taskRef, runId, sessionId, generation, question, ttlMs, telegramMessageId = null, nowMs = Date.now(), lifetimeMode = process.env.ACP_GATE_LIFETIME_MODE || OPERATOR_WAIT_MODE }) {
  const boundedTtl = Math.min(Math.max(Number(ttlMs) || DEFAULT_TTL_MS, 1000), MAX_TTL_MS);
  const sessionIdSha = sha12(sessionId);
  const operatorWait = lifetimeMode === OPERATOR_WAIT_MODE;
  const decision = {
    ...
    state: "REGISTERED",
    // lifetime law: operator_wait = NO auto-expiry; legacy ttl fields kept for
    // backward compatibility ONLY (never enforced on this path).
    lifetime_mode: operatorWait ? OPERATOR_WAIT_MODE : "bounded_ttl",
    created_at: new Date(nowMs).toISOString(),
    expires_at: operatorWait ? null : new Date(nowMs + boundedTtl).toISOString(),
    ttl_ms: operatorWait ? null : boundedTtl,
    transport: null,
...
export function admitGateCallback(store, cb, { taskRef, sessionId, generation, nowMs = Date.now() }) {
  ...
  // Lifetime law: elapsed time alone is NEVER terminal on the operator-wait
  // path (expires_at is legacy/null there). Legacy bounded_ttl decisions keep
  // their EXPIRED fence (old fixtures/evidence remain valid).
  if (d.lifetime_mode !== OPERATOR_WAIT_MODE && d.expires_at && nowMs >= Date.parse(d.expires_at)) { appendHistory(d, "EXPIRED", nowMs); return { ok: false, reason: "GATE_DECISION_EXPIRED" }; }
...
export function markNoAnswer(store, decisionId, { nowMs = Date.now() } = {}) {
  const d = store.decisions.find((x) => x.decision_id === decisionId);
  if (!d) return { ok: false, reason: "GATE_DECISION_UNKNOWN" };
  if (d.lifetime_mode === OPERATOR_WAIT_MODE) return { ok: false, reason: "GATE_OPERATOR_WAIT_NO_AUTO_EXPIRY" };
...
/**
 * CANCELLED — EXPLICIT operator/driver cancellation of a pending gate.
 * Never generated by a timer. Auditable via history; terminal.
 */
export function markCancelled(store, decisionId, { nowMs = Date.now(), by = null, reason = null } = {}) {
  const d = store.decisions.find((x) => x.decision_id === decisionId);
  if (!d) return { ok: false, reason: "GATE_DECISION_UNKNOWN" };
  if (!["REGISTERED", "NOTIFIED"].includes(d.state)) return { ok: false, reason: `GATE_BAD_TRANSITION_${d.state}` };
  d.cancelled_by = by ? String(by).slice(0, 64) : null;
  d.cancelled_reason = reason ? String(reason).slice(0, 120) : null;
  appendHistory(d, "CANCELLED", nowMs);
  return { ok: true, decision: d };
}

/**
 * SUPERSEDED — EXPLICIT canonical supersession: a NEW gate (higher
 * generation, same task/run) supersedes this one through a persisted
 * transition. Never implicit, never silent, never time-based.
 */
export function markSuperseded(store, decisionId, { nowMs = Date.now(), supersededBy = null } = {}) {
  const d = store.decisions.find((x) => x.decision_id === decisionId);
  if (!d) return { ok: false, reason: "GATE_DECISION_UNKNOWN" };
  if (!["REGISTERED", "NOTIFIED"].includes(d.state)) return { ok: false, reason: `GATE_BAD_TRANSITION_${d.state}` };
  d.superseded_by = supersededBy ? String(supersededBy).slice(0, 64) : null;
  appendHistory(d, "SUPERSEDED", nowMs);
  return { ok: true, decision: d };
}
