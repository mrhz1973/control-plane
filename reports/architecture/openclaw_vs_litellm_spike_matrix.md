...
| 3 | Qwen local / Ollama path | `NOT_TESTED` (HOME host offline; OpenClaw path intact but not exercised) | `CONFIG_DEFINED_NOT_RUNTIME_PROVEN` (`planner-qwen-pilot` → Qwen 3.8 37B placeholder) | Runtime deferred; no 27B; no model download; not blocking GLM/Codex |
...
| 7 | Retry / fallback controllability | `PROVEN_OFFLINE` (adapter synthesizes no provider fallback) | `CONFIG_DEFINED_NOT_RUNTIME_PROVEN` (pilot template has no fallback chain) | D-0024 caps: retry=0, planner fallback=0, gateway fallback=0 |
| 8 | n8n integration shape | `NOT_TESTED` (n8n unchanged) | `NOT_TESTED` (no n8n mutation) | Production routing not changed |
| 9 | Operational complexity | `PROVEN_OFFLINE` (existing install retained; Phase B still separate) | `CONFIG_DEFINED_NOT_RUNTIME_PROVEN` (template only; no install/start in D-0023) | LiteLLM remains candidate |
...
| 12 | Current evidence status | Offline path + Phase B auth remain as previously recorded | Offline/config spike PASS only; D-0024 GLM+Codex STAGED | Qwen runtime deferred |

## Decision snapshot (offline)

- LiteLLM is a viable **candidate** gateway for Architecture v3 once the current remote-planner runtime prerequisites pass.
...
- Current D-0024 priority may consume at most **2 inferences total**: 1×GLM Coding Plan + 1×Codex OAuth, each with retry/fallback zero.
- Qwen runtime is `DEFERRED_NOT_BLOCKING_CURRENT_REMOTE_PATH` and has a current inference budget of 0.
