...
async function testStateMachineAndFences() {
  ...
  // stale: expired decision — LEGACY bounded_ttl mode (operator_wait never expires)
  const store3 = loadGateStore(storePath);
  const d3 = registerGateDecision(store3, { taskRef: TASK_REF, runId: RUN_ID + "C", sessionId, generation: 1, question: { prompt: "x", options: ["A", "B", "C"] }, ttlMs: 1000, nowMs: Date.now() - 5000, lifetimeMode: "bounded_ttl" });
  ...
  // LEGACY bounded_ttl decision (operator_wait never auto-NO_ANSWER — asserted
  // in the OPERATOR_WAIT_NO_AUTO_EXPIRY check below).
  const store4 = loadGateStore(storePath);
  const dLegacy = registerGateDecision(store4, { taskRef: TASK_REF, runId: RUN_ID + "L", sessionId, generation: 1, question: { prompt: "x", options: ["A", "B", "C"] }, ttlMs: 1000, lifetimeMode: "bounded_ttl" });
  markNotified(store4, dLegacy.decision_id, { transport: "synthetic", messageId: "m-l" });
  const na = markNoAnswer(store4, dLegacy.decision_id);
  check("NO_DEFAULT_ANSWER", na.ok === true && na.decision.state === "NO_ANSWER" && na.decision.selected_option === null, na.decision.state);
  // operator_wait law: NO_ANSWER can never be auto-generated by the clock
  const d4 = registerGateDecision(store4, { taskRef: TASK_REF, runId: RUN_ID + "D", sessionId, generation: 1, question: { prompt: "x", options: ["A", "B", "C"] }, ttlMs: 1000 });
  markNotified(store4, d4.decision_id, { transport: "synthetic", messageId: "m-4" });
  const naBlocked = markNoAnswer(store4, d4.decision_id);
  check("OPERATOR_WAIT_NO_AUTO_EXPIRY", naBlocked.ok === false && naBlocked.reason === "GATE_OPERATOR_WAIT_NO_AUTO_EXPIRY", naBlocked.reason);
}
...
async function testAdapterEndToEndSynthetic() {
  ...
  try {
    ...
    // silent transport -> the PERSISTENT operator wait does NOT auto-terminate:
    // status stays PENDING (never a default, never a clock-made failure).
    // (The terminal NO_ANSWER path exists ONLY for legacy bounded_ttl gates —
    // asserted in the state-machine section.)
    const gs3Path = path.join(tmpdir(), "gs3.json");
    const child3 = startServer({ ACP_GATE_STORE_PATH: gs3Path, ACP_GATE_SPOOL_DIR: spool, ACP_GATE_TRANSPORT_MODULE: T_SILENT, ACP_GATE_TTL_MS: "15000", ACP_GATE_STATUS_POLL_SLICE_MS: "1000" });
    try {
      ...
      await new Promise((r) => setTimeout(r, 1200)); // transport wait returns TIMEOUT; waiter re-arms
      ...
      check("ADAPTER_SILENT_NO_ANSWER", st3Out.status === "PENDING" && st3Out.option === undefined, `status=${st3Out.status}`);
      const gs = JSON.parse(fs.readFileSync(gs3Path, "utf8"));
      const d3s = gs.decisions.find((x) => x.decision_id === out3.decision_id);
      check("SILENT_GATE_STAYS_PENDING_PERSISTED", d3s && ["NOTIFIED", "PENDING"].includes(d3s.state) && d3s.selected_option === null, d3s?.state);
    } finally { child3.kill(); }
